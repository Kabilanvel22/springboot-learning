...
import com.example.demorepository.Employeerepository;
...
//return employeeRepository.findAll(Sort.by(field));
	...
	public List<Employee> getEmployeesByDept(String dept) {
		return employeerepository.findAllEmployeesByDept(dept);
		
	}

	public List<Employee> getEmployeesByNamePrefix(String prefix) {
		return employeerepository.findByNameStartingWith(prefix);
		
	}

	public List<Employee> getEmployeesByNameSuffix(String suffix) {
	return employeerepository.findByNameEndingWith(suffix);
	}

	@Transactional
	public int updateEmployeeSalary(double sal, long id) {
		int result=employeerepository.updateEmployeeSalary(sal, id);
		return result;
	}
